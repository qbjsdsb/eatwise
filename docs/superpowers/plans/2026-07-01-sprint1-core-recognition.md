...
**Tech Stack:** Flutter 3.x (Dart ^3.10), drift ^2.34.0, sqlite3 ^3.3.2 (build hooks: sqlite3mc), flutter_secure_storage ^10.3.1, flutter_riverpod ^3.3.1, go_router ^17.2, openai_dart ^7.0.0, image_picker, flutter_image_compress, connectivity_plus
...
修改 `android/app/build.gradle.kts`，在 `defaultConfig` 块中将 `minSdkVersion` 由模板默认的 `flutter.minSdkVersion`（21）改为 `23`：
...
android {
    ...
    defaultConfig {
        ...
        minSdkVersion = 23  // flutter_secure_storage 10.x 要求（原 flutter.minSdkVersion 默认 21）
