...
**Tech Stack:** Flutter 3.x (Dart ^3.10), drift ^2.34.0, sqlite3 ^3.3.2 (build hooks: sqlite3mc), flutter_secure_storage ^10.3.1, flutter_riverpod ^3.3.1, go_router ^17.2.0, openai_dart ^7.0.0, image_picker, flutter_image_compress, connectivity_plus
...
- Modify: `android/app/build.gradle.kts` (minSdk 23)
...
environment:
  sdk: ^3.10.0  # drift 2.34.0 要求 Min Dart SDK 3.10（pub.dev 已核实）
...
dependencies:
  ...
  # 状态管理 + 路由
  flutter_riverpod: ^3.3.1  # 3.3.2 仅为 3.3.2-dev.2 prerelease，最新稳定版是 3.3.1（pub.dev 已核实）
  go_router: ^17.2.0  # pub 的 ^ 约束需完整 major.minor.patch，原 "^17.2" 缺 patch 号会报错
...
dev_dependencies:
  ...
  # drift 代码生成 + 迁移工具
  drift_dev: ^2.34.0  # 2.34.1 不存在，最新稳定版是 2.34.0（pub.dev 已核实）
  build_runner: ^2.15.0
...
> 注意：Flutter 3.29+（2025-04）起 `flutter create` 默认生成 Kotlin DSL 的 `android/app/build.gradle.kts`（而非 Groovy 的 `build.gradle`）。本项目按 Kotlin DSL 处理；若你的 Flutter 版本仍生成 `build.gradle`，按 Groovy 语法等价修改即可。

修改 `android/app/build.gradle.kts`，在 `defaultConfig` 块中将 `minSdk` 由模板默认的 `flutter.minSdkVersion`（21）改为 `23`：

> 注意：Flutter 3.29+ 生成的 Kotlin DSL 模板使用属性赋值式语法（`minSdk = ...` / `targetSdk = ...` / `versionCode = flutter.versionCode` / `versionName = flutter.versionName`），**不是**旧 Groovy 的 `minSdkVersion = ...` / `versionCode = flutterVersionCode.toInt()`。下列代码与 Flutter 3.44.4 实测生成模板一致，仅把 `minSdk` 的值改为 `23`，其余行保持模板原样不动。

```kotlin
android {
    // ...其余保持模板生成内容不变（namespace/compileSdk/ndkVersion/compileOptions 等）...
    defaultConfig {
        applicationId = "com.eatwise.eatwise"
        minSdk = 23  // flutter_secure_storage 10.x 要求（原 flutter.minSdkVersion 默认 21）
...
```bash
git add pubspec.yaml build.yaml android/app/build.gradle.kts ios/Runner/Info.plist lib/ test/ drift_schemas/
...
**6. 第四轮严审发现并修复（P–V，逐项核实 pub.dev / 官方源码）：**

| 编号 | 问题 | 修复 | 核实来源 |
|---|---|---|---|
| P | `drift_dev: ^2.34.1` — 2.34.1 版本不存在，pub.dev 最新仅 2.34.0，`^2.34.1` 约束会解析失败 | 改为 `^2.34.0` | pub.dev/packages/drift_dev |
| Q | `sdk: ^3.5.0` — drift 2.34.0 的 Min Dart SDK = 3.10，`^3.5.0` 下限过低会导致依赖解析冲突 | 改为 `^3.10.0` | pub.dev/packages/drift/versions（Min Dart SDK 字段） |
| R | Riverpod 3.x 将 `StateNotifier`/`StateNotifierProvider`/`StateProvider`/`ChangeNotifierProvider` 移至 `legacy.dart`，主入口 `flutter_riverpod.dart` 不再导出 — `recognize_controller.dart` 的 `extends StateNotifier` 会编译失败 | `recognize_controller.dart` 导入改为 `package:flutter_riverpod/legacy.dart`（`ProviderScope`/`Provider`/`FutureProvider` 仍在主入口，`main.dart`/`providers.dart` 无需改） | riverpod.dev/zh-hans/docs/3.0_migration（官方迁移指南） |
| S | Task 5 QwenVlProvider + Glm4vProvider 使用的 openai_dart API **全部错误**（`OpenAIConfig(apiKey:)` 不存在 / `client.chat.createChatCompletion(request:)` 方法名错 / `CreateChatCompletionRequest`/`ChatCompletionMessage`/`ChatCompletionModel` 类名错 / `content.first.text` 访问路径错 / `OpenAIClientException` 异常类不存在 / `const ResponseFormat.jsonObject()` 对静态方法加 const 编译错误） | 完全重写两个 Provider，使用已核实的 openai_dart 7.0 API：`OpenAIClient(config: OpenAIConfig(authProvider: ApiKeyProvider(apiKey), baseUrl:))` / `client.chat.completions.create(ChatCompletionCreateRequest(model: 纯String, responseFormat: ResponseFormat.jsonObject()（无 const）, messages: [ChatMessage.system(String), ChatMessage.user(UserMessageContent.parts([TextContentPart, ContentPart.imageBase64(data:,mediaType:)]))]))` / `response.text`（String?）/ 分级异常（RateLimitException.retryAfter / AuthenticationException / PermissionDeniedException / RequestTimeoutException / ConnectionException / InternalServerException / ApiException.statusCode / OpenAIException） | github.com/davidmigloz/ai_clients_dart/packages/openai_dart/example/（vision_example.dart + error_handling_example.dart）+ 源码 content_part.dart / response_format.dart / config.dart |
| T | Flutter 3.29+（2025-04）起 `flutter create` 默认生成 Kotlin DSL 的 `android/app/build.gradle.kts`（非 Groovy 的 `build.gradle`）— 计划中三处引用 `build.gradle` + Groovy 语法 `minSdkVersion 23`（无 `=`）会与实际生成的文件不匹配 | 三处统一改为 `build.gradle.kts` + Kotlin DSL 语法 `minSdkVersion = 23`（含 `applicationId =`/`targetSdkVersion =`/`versionCode =`/`versionName =`），并加注释说明 Flutter 3.29+ 默认行为 | codewithandrea.com/articles/flutter-android-gradle-kts + flutter_tools/templates/app/android-kotlin/ |
| V | `flutter_riverpod: ^3.3.2` — 3.3.2 仅为 `3.3.2-dev.2` prerelease，最新稳定版是 3.3.1，`^3.3.2` 约束会解析失败 | 改为 `^3.3.1`（pubspec.yaml + Tech Stack 行一致更新） | pub.dev/packages/flutter_riverpod（Versions 页确认 3.3.1 稳定 / 3.3.2-dev.2 prerelease） |

**第四轮额外核实项（无问题，记录备查）：**
- pubspec.yaml 中其余 13 个 `^` 约束（sqlite3/fl_chart/connectivity_plus/path_provider/path/uuid/flutter_lints/build_runner/mocktail/image_picker/flutter_image_compress/flutter_secure_storage/go_router）全部能解析到真实稳定版 ✅
- `ResponseFormat.jsonObject()` 为 `sealed class ResponseFormat` 的静态工厂方法（返回 `const JsonObjectResponseFormat()`），**不可加 `const` 前缀**（原代码 `const ResponseFormat.jsonObject()` 会编译错误）— 已在 Issue S 重写中修正 ✅
- `OpenAIConfig` 构造函数参数为 `authProvider`（AuthProvider?），**无 `apiKey` 参数**（原代码 `OpenAIConfig(apiKey:, baseUrl:)` 会编译错误）— 已在 Issue S 重写中修正 ✅
- `ContentPart.imageBase64({required String data, required String mediaType})` 接收原始 base64 字符串（非 `data:URL`），SDK 内部拼接 `data:$mediaType;base64,$data` — 已在 Issue S 重写中正确使用 ✅
- `ChatMessage.user` 接受 `String`（纯文本）或 `UserMessageContent`（多模态）；多模态必须用 `UserMessageContent.parts(List<ContentPart>)` 包装，不可传裸 `List` — 已在 Issue S 重写中正确使用 ✅
- openai_dart 7.0 异常层级：`OpenAIException`（基类，有 `.message`）→ `ApiException`（有 `.statusCode`/`.type`/`.code`/`.param`/`.requestId`）→ 各 HTTP 状态码子类（`AuthenticationException` 401 / `PermissionDeniedException` 403 / `RateLimitException` 429 含 `.retryAfter` / `BadRequestException` 400 / `NotFoundException` 404 / `InternalServerException` 5xx）；另 `RequestTimeoutException`（含 `.timeout`）/ `ConnectionException`（含 `.url`）为传输层异常 — catch 顺序须从具体到通用 ✅
- Riverpod 3.x 迁移：仅 `StateNotifier`/`StateNotifierProvider`/`StateProvider`/`ChangeNotifierProvider` 移至 `legacy.dart`；`ProviderScope`/`Provider`/`FutureProvider`/`StreamProvider`/`NotifierProvider`/`AsyncNotifierProvider` 仍在主入口 — 本计划 `main.dart`（ProviderScope）/ `providers.dart`（Provider/FutureProvider）无需改，仅 `recognize_controller.dart`（StateNotifier）需 legacy 导入 ✅

**7. 冒烟预演发现并修复（W/X，沙箱 Flutter 3.44.4 + Dart 3.12.2 实测）：**

| 编号 | 问题 | 修复 | 核实来源 |
|---|---|---|---|
| W | Issue T 修复时将 build.gradle.kts 语法写为 `minSdkVersion = 23` / `targetSdkVersion = flutter.targetSdkVersion` / `versionCode = flutterVersionCode.toInt()` / `versionName = flutterVersionName` —— 但 Flutter 3.44.4 实测生成的 Kotlin DSL 模板使用**属性赋值式短名**：`minSdk = ...` / `targetSdk = ...` / `versionCode = flutter.versionCode` / `versionName = flutter.versionName`。旧写法 `minSdkVersion =` 在 Kotlin DSL 中不是合法属性，会编译失败 | Task 1 Step 4 改为 `minSdk = 23` / `targetSdk = flutter.targetSdkVersion` / `versionCode = flutter.versionCode` / `versionName = flutter.versionName` | 沙箱 `flutter create` 实测生成的 android/app/build.gradle.kts（Flutter 3.44.4） |
| X | `go_router: ^17.2` —— pub 的 `^` 约束语法要求 `^` 后跟完整 `major.minor.patch`，`^17.2` 缺 patch 号会报 `Invalid version constraint: Expected version number after "^" in "^17.2"`，`flutter pub get` 失败 | 改为 `^17.2.0`（pubspec.yaml 块 + Tech Stack 行一致更新）；实测 pubspec.lock 解析到 17.3.0 | 沙箱 `flutter pub get` 实测报错 + 修复后成功（121 依赖解析） |

**冒烟预演额外验证项（沙箱实测，全部通过）：**
- Flutter 3.44.4 + Dart 3.12.2 安装成功，满足 drift 2.34.0（Min Dart 3.10）+ Flutter 3.29+（生成 build.gradle.kts）要求 ✅
- pubspec.yaml 全部 16 个依赖解析成功（121 个包），pubspec.lock 确认关键版本：drift 2.34.0 / drift_dev 2.34.0 / sqlite3 3.3.4 / flutter_riverpod 3.3.2（3.3.2-dev.2 prerelease 被跳过）/ go_router 17.3.0 / openai_dart 7.0.0 ✅
- sqlite3mc build hooks 工作正常：`dart run build_runner build` 退出码 0，生成 database.g.dart（8 outputs），sqlite3mc 编译通过 ✅
- openai_dart 7.0 API 编译验证通过：OpenAIClient/OpenAIConfig/ApiKeyProvider/ChatMessage.system/ChatMessage.user 多模态/ContentPart.imageBase64/ResponseFormat.jsonObject()/ChatCompletionCreateRequest/全异常层级 —— 2 个测试全过 ✅
- **真实 API 冒烟测试通过**（百炼 OpenAI 兼容 endpoint + 用户提供 key）：qwen-vl-max 识别苹果返回 `{"dish_name":"苹果","is_single_item":true,"confidence":0.99,"estimated_weight_g_mid":180}`（confidence 0.99 超过成功标准 0.9）；qwen3-vl-flash 识别苹果返回 confidence 0.98 / weight 200g —— Sprint 1 成功标准端到端验证通过 ✅
- **qwen3-vl-flash 已知问题**：对无法识别的图片（如占位符图），flash 模型会产生畸形超长数字输出导致 JSON 截断解析失败 —— 印证 Task 5 `vision_response_parser_test.dart`（JSON 解析容错）的必要性；生产环境建议默认用 qwen-vl-max，flash 仅作低成本备选 ✅

**8. 正式实施发现并修复（Y–GG，Task 2–7 编译验证实测）：**

| 编号 | 问题 | 修复 | 核实来源 |
|---|---|---|---|
| Y | Task 2 database.dart 的 `databaseProvider = FutureProvider<...>` 报 `FutureProvider` undefined —— 漏 import（FutureProvider/Provider 在 flutter_riverpod 主入口，非 legacy） | 添加 `import 'package:flutter_riverpod/flutter_riverpod.dart';` | flutter analyze 实测 |
| Z | Task 2 database_test.dart `isNull` ambiguous —— drift 和 matcher 包都导出 `isNull` | `import 'package:drift/drift.dart' hide isNull;`（Task 4 food_seed_importer_test 同理需 `hide isNull, isNotNull`，两个符号都冲突） | flutter analyze 实测 |
| AA | Task 2 database_test.dart drift 旧 API `db.table.where()` 不存在（3 处）—— drift 2.x 查询必须用 `db.select(db.table)..where((t) => ...)` 扩展方法语法 | 3 处查询改为 `(db.select(db.profiles)..where((p) => p.id.equals(1))).getSingle()` 语法 | drift 2.34 文档 + analyze |
| BB | Task 2 `EatWiseDatabase(QueryExecutor executor) : super(executor)` 可简化为 super parameter | 改为 `EatWiseDatabase(super.executor);` | Dart 3 super parameters |
| CC | Task 7 meal_log_repository.dart `getTotalCaloriesByDate` 的 `fold(0.0, (sum, m) => sum + m.actualCalories)` 在 async `Future<double>` 函数中泛型 T 被推断为 `FutureOr<double>`（async 函数返回类型上下文是 `FutureOr<T>`），导致 `sum +` 未定义 | 显式 `fold<double>(0.0, ...)` 强制 T=double，阻断 FutureOr 推断 | flutter analyze 实测 |
| DD | Task 7 recognize_page.dart 的 `foodItemRepo: await ref.read(...)` 写在 `MaterialPageRoute(builder: (_) => ...)` 同步闭包内 → `await_in_wrong_context` error | await 提到 `Navigator.push` 之前，存局部变量 `foodItemRepo` 传入 builder（并补 `if (!mounted) return;` 守卫） | flutter analyze 实测 |
| EE | Task 7 recognize_page.dart `controller.state` 触发 `invalid_use_of_protected_member` —— StateNotifier.state 标注 `@protected`，外部非子类读取违规 | RecognizeController 加公开 getter `RecognizeUiState get current => state;`，页面改用 `controller.current` | state_notifier 源码 + analyze |
| FF | Task 7 `RecognizeState_`（尾下划线，原为避 enum `RecognizeState` 冲突）触发 `camel_case_types` info | 重命名为 `RecognizeUiState`（recognize_page 用 `final state = controller.current;` 类型推断，无需改） | flutter analyze 实测 |
| GG | Task 7 RecognizeController 的 `_foodItemRepo` 字段未使用（营养查库全委托给 `_nutritionLookup`，repo 为死代码）→ `unused_field` | 移除字段 + 构造参数 + recognize_page 调用方 arg + `food_item_repository.dart` import | flutter analyze 实测 |

**正式实施额外清理（unused_import，达成 analyze 0 issues）：**
- recognize_controller.dart 移除未用 `dart:io`（`FlutterImageCompress.compressWithFile` 接收 String path，非 File 对象）
- recognize_page.dart 移除未用 `ai/nutrition_lookup.dart` + `ai/vision_provider.dart`（类型经 `state` 推断，无需显式 import）
- providers.dart 移除未用 `ai/vision_provider.dart`（QwenVlProvider/Glm4vProvider 为具体类型，无需抽象基类 import）
- nutrition_lookup.dart 移除未用 `data/database/database.dart`（FoodItem 经 food_item_repository 传递可用）

**正式实施验证结果（沙箱 Flutter 3.44.4 + Dart 3.12.2）：**
- `flutter analyze`：**No issues found!**（0 errors / 0 warnings / 0 info）✅
- `flutter test`：**30 tests passed**（Task 1–6 全部测试回归通过）✅
- Task 7 端到端编译验证通过（6 新文件：meal_log_repository / recognize_controller / calibration_page / providers / recognize_page / dashboard_page + app.dart 路由覆写为 DashboardPage）✅
- Task 7 Step 9 端到端手动验证（flutter run + 真实 API）需真机/模拟器，沙箱无设备 —— 编译已验证通过，待用户在本地设备执行 `flutter run --dart-define=QWEN_API_KEY=... --dart-define=QWEN_BASE_URL=...` 完成拍照→识别→校准→写库闭环
