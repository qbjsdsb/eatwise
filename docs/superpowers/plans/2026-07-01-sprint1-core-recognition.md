...
Future<String> _getOrCreatePassphrase() async {
  ...
  // 生成 32 字节随机密钥，hex 编码作为 passphrase
  final random = DateTime.now().microsecondsSinceEpoch;
  final buffer = StringBuffer();
  for (var i = 0; i < 32; i++) {
    buffer.write((random + i * 7919).toRadixString(16).padLeft(2, '0').substring(0, 2));
  }
  final passphrase = buffer.toString();
...
（原 Step 13 测试内容前移到 Step 12，后续编号依次 -1）
...
void main() {
  ...
  test('解析 Sanotsu JSON：字段映射正确', () {
    ...
    // 番茄：去除 [西红柿] 后缀
    expect(items[0].name, '番茄');
    expect(items[0].caloriesPer100g, 18);
    expect(items[0].ediblePercent, 97);

    // 马铃薯：去除 (土豆,洋芋) 后缀
    expect(items[1].name, '马铃薯');
    expect(items[1].carbsPer100g, 16.5);

    // 花生油："—" → null，"Tr" → 0.05
    expect(items[2].name, '花生油');
    expect(items[2].proteinPer100g, isNull);
    expect(items[2].carbsPer100g, 0.05);

    // 苹果：edible 空串 → null
    expect(items[3].name, '苹果');
    expect(items[3].ediblePercent, isNull);
  });
...
class FoodSeedImporter {
  ...
  static FoodItemsCompanion _parseItem(Map<String, dynamic> raw) {
    ...
    return FoodItemsCompanion.insert(
      name: name,
      defaultServingG: const Constant(100),
      caloriesPer100g: _parseDouble(raw['energyKCal']),
      proteinPer100g: _parseNullableDouble(raw['protein']),
...
JSON schema：
{
  ...
  "cooking_method": "烹饪方式: steam/boil/cold/toss/roast/stir-fry/pan-fry/deep-fry/braise 之一",
...
class QwenVlProvider implements VisionProvider {
  ...
  Future<VisionRecognitionResult> recognize(String imageBase64) async {
    try {
      final response = await _client.chat.createChatCompletion(
...
- [ ] **Step 6: 实现 GLM-4V-Plus 容灾 Provider**
...
class Glm4vProvider implements VisionProvider {
  ...
  @override
  Future<VisionRecognitionResult> recognize(String imageBase64) async {
    // 复用 QwenVlProvider 的实现逻辑（仅 client/modelName 不同）
    // 用 mixin 或提取公共方法避免重复（此处为 Sprint 1 简化，直接复用 QwenVlProvider 的私有逻辑）
    final qwenLike = QwenVlProvider(
      apiKey: '', // 不使用，直接传入 client
      baseUrl: '',
      modelName: _modelName,
    );
    // 注意：QwenVlProvider 构造函数会创建新 client，这里需重构
    // Sprint 1 简化：直接复制 recognize 逻辑，仅替换 client 和 model
    // TODO: 重构为提取公共 _recognizeWithClient 方法
    ...
    try {
      ...
      throw VisionRecognitionException('GLM-4V-Plus 错误: $e', retryable: false);
    }
  }
}
```

- [ ] **Step 7: 重构 - 提取公共 recognize 逻辑避免重复**

修改 `lib/ai/qwen_vl_provider.dart`，提取公共方法：
...
- [ ] **Step 8: 重写 Glm4vProvider 复用公共方法**

覆盖 `lib/ai/glm_4v_provider.dart`：
...
class NutritionLookup {
  ...
  Future<NutritionResult?> lookupSingleItem({
    ...
    return NutritionResult(
      ...
      proteinG: (food.proteinPer100g ?? 0) * servingG / 100,
      fatG: (food.fatPer100g ?? 0) * servingG / 100,
      carbsG: (food.carbsPer100g ?? 0) * servingG / 100,
      oilG: 0,
    );
  }
  ...
  Future<CompositeNutritionResult> lookupCompositeDish({
    ...
    for (final comp in components) {
      ...
      totalProtein += (food.proteinPer100g ?? 0) * g / 100;
      totalFat += (food.fatPer100g ?? 0) * g / 100;
      totalCarbs += (food.carbsPer100g ?? 0) * g / 100;
      ...
      Navigator.of(context).push(MaterialPageRoute(
        builder: (_) => CalibrationPage(
          ...
          onConfirm: (servingG, calories, protein, fat, carbs, {componentsSnapshot}) async {
            ...
            // 写入 meal_log（Sprint 1 简化：food_item_id 用查库命中或 0 占位）
            final result = state.recognitionResult!;
            await mealRepo.insertMealLog(
              date: _todayLocalDate(),
              mealType: 'snack', // Sprint 2 加餐次选择 UI
              foodItemId: state.singleNutrition?.foodItemId ?? 0,
...
class DashboardPage extends ConsumerWidget {
  ...
  Future<double> _getTodayCalories(WidgetRef ref) async {
    final repo = await ref.read(mealLogRepoProvider.future);
    final now = DateTime.now();
...
- [ ] **Step 7: 修正 dashboard 引用 providers.dart**

`lib/features/dashboard/dashboard_page.dart` 引用了 `providers.dart`，但 providers.dart 在 recognize 目录。需调整。创建 `lib/features/providers.dart` 统一出口：

创建 `lib/features/providers.dart`：

```dart
export '../data/database/database.dart' show databaseProvider;
export 'recognize/providers.dart';
```

修改 `lib/features/dashboard/dashboard_page.dart` 的 import：
...
并将 `_getTodayCalories` 改用 `recognize.mealLogRepoProvider`：
...
- `cookingOilCoefficients` 在 Task 6 定义，key 使用 `stir-fry`/`boil` 等 — 与 prompt 中 cooking_method 取值一致 ✅

**4. 已知简化项（Sprint 1 故意简化，Sprint 2/3 完善）：**
