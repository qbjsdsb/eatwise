...
class PackageNutritionOcrParser {
  ...
  static final List<RegExp> _carbsPatterns = [
    ...
    // 用负向回视断言防误匹配两类"糖"前缀字：
    //   1. 修饰词声称：低糖/无糖/加糖/含糖/少糖/减糖/高糖（"低无加含少减高"7字）
    //   2. 多字糖类配料名（M8 修复）：蔗糖/果糖/乳糖/麦芽糖/葡萄糖
    //      ——这些是配料表术语，营养成分表行只标单字"糖"
    //      糖前缀字分别为：蔗/果/乳/芽/萄（"麦芽糖"前是"芽"，"葡萄糖"前是"萄"）
    //      额外加 麦/葡 双字防 OCR 误读变体，无副作用
    // 注意 Dart RegExp 支持 (?<!...) 固定宽度回视（Dart 2.4+）
    RegExp(r'(?<![低无加含少减高果乳蔗麦芽葡萄])糖\s*[：:、\s]*\s*(\d+(?:\.\d+)?)\s*g', caseSensitive: false),
  ];
}
