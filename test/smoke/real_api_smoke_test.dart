...
void main() {
  ...
  group('Sprint 1 终端 E2E（真实 API + 内存 DB）', () {
    test('苹果图 → Qwen-VL → 查库 → 写 meal_log → 今日热量增加',
        skip: canRun ? false : '需 QWEN_API_KEY + /tmp/apple.jpg', () async {
      // ── 1. 内存 DB + 种子导入 ──
...
''';
      ...
      // ignore: avoid_print
      print('[2/6] Qwen-VL-max 识别：${result.dishName} '
      ...
      // ignore: avoid_print
      print('[3/6] 查库回填：${nutrition?.calories.toStringAsFixed(2)} kcal '
      ...
      // ignore: avoid_print
      print('[4/6] 校准页：confidence ${result.confidence} ${canSkip ? "≥ 0.85 → 一键记录" : "< 0.85"}');
      ...
      // ignore: avoid_print
      print('[6/6] 今日热量：${initialCalories.toStringAsFixed(1)} → ${finalCalories.toStringAsFixed(1)} kcal '
          '(+${(finalCalories - initialCalories).toStringAsFixed(2)})');
      expect(finalCalories, greaterThan(initialCalories),
          reason: '写库后今日热量应增加');
    ...
    test('qwen3-vl-flash 备选模型识别苹果',
      ...
      final result = await provider.recognize(imageBase64);
      // ignore: avoid_print
      print('qwen3-vl-flash: ${result.dishName} confidence=${result.confidence} '
