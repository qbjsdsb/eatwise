...
void main() {
  ...
  test('profile 单行初始化', () async {
    final profile = await (db.select(db.profiles)..where((p) => p.id.equals(1))).getSingle();
  ...
  test('food_item 插入与查询', () async {
    ...
    final item = await (db.select(db.foodItems)..where((f) => f.id.equals(id))).getSingle();
    expect(item.name, '苹果');
  ...
  test('meal_log 外键关联 food_item', () async {
    ...
    final meal = await (db.select(db.mealLogs)..where((m) => m.id.equals(mealId))).getSingle();
