# M18 多菜场景 AI 推理可见性 + 三路径一致性（v0.19.0）

> **触发**：用户反馈"比较复杂的场景比如有好几个食物的情况下，没有 AI 推理的过程，精准度我也有一定程度的怀疑，是不是正确安装 AI 的计算提交并且显示的，希望还能继续改进，严谨仔细，一定不能出问题"
> **Skill 组合**：brainstorming（方案发散）+ test-driven-development（Red-Green-Refactor 严格 TDD）
> **用户决策**：范围 = AI 可见性 + 三路径一致性；形态 = 完整 AI 估算卡片
...
| 决策点 | 选择 | 理由 |
|--------|------|------|
| 改进范围 | **AI 可见性 + 三路径一致性** | 用户要求"严谨仔细，一定不能出问题"，三路径不一致是已知技术债，趁此机会统一 |
