...
- [ ] 复合菜预览/记录路径优先级一致：`_buildNutritionPreview` 与 `_confirmWithServing` 在"包装+宏量全0+aiFallback"场景都走 AI 优先
- [ ] 复合菜 AI 优先路径含用油量：用户拖动用油量滑块时预览热量+脂肪实时变化
...
- [ ] 新增 5 个测试文件覆盖 A 类 5 个 bug 修复
- [ ] A 类 commit 完成，flutter analyze + flutter test 通过
