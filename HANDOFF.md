...
- **当前版本**：0.20.1+32（pubspec.yaml）—— 已发布 v0.20.0 GitHub Release（M19+M20 严肃 push + tag）；v0.20.1 为项目全面审查 P0+P1 修复（HANDOFF 同步 + 测试缺口补全）
...
最终验证：flutter analyze No issues / flutter test 1107 passed（基线 1093 → +14 问题1 + +28问题2 = 1107，0 回归）/ 6+1 硬约束满足（minify=false / shrink=false / minSdk=31 等未碰）/ 0 回归。**已打 tag v0.25.0 + push + GitHub release**。

**两个 UX Bug 修复完成（2026-07-06，已发版 v0.25.0）—— 方案 A 双 commit**：用户报告两个问题：(1) 删除食物时撤销横幅"存在非常久"；(2) AI 智能推荐刚打开软件时经常加载失败。用户决策"两个一起全面修（推荐）"+"就方案 a 吧"（分问题双 commit）。
...
## M20 Google Lens 风图标 + 识别思考流程 UI（2026-07-05）—— v0.20.0

### 任务来源
用户反馈"图标还是太难看，希望像谷歌公司会做的样子，精致一点"+"拍照识别等候界面很单调，希望把 AI 思考流程放出来，也很美观"。用户后续指令"包括我 M19 的改动也要一并 push 和 tag"。

### 根因分析
- **图标**：M17 同心圆环+中心圆点抽象几何，用户反馈"还是太难看"，不够谷歌风
- **等候界面**：极简 Card + CircularProgressIndicator + "识别中…" 文本，底层 `RecognizeController` 已定义 6 个细粒度状态但 UI 完全没读取，无阶段感无进度感

### 实现方案（双线并行）

**图标重设计（Google Lens 风）**：
- 保留 M17 紫橙渐变背景（#6750A4 → #FF6E40）
- 前景重写：四角 L 形角标（取景框）+ 中心苹果剪影（食物识别符号）+ 水平扫描线（AI 扫描中）
- 几何：画布 108×108dp，四角 L 在 (29,29)-(79,79) 取景框四角，每角 12×12dp 描边 4dp；苹果剪影直径 20dp 中心 (54,54)；扫描线 y=54 x=33-75 描边 2dp
- Round 0 计划用 Seedream AI 生成 4 张参考图，沙箱无 ARK_API_KEY 跳过，直接基于设计规格手工实现 vector drawable

**等候界面升级（4 阶段进度卡片）**：
- 新建 `RecognizeProgressCard` widget：4 阶段竖向列表 + 顶部进度条
- 4 阶段映射 `RecognizeState`：pickingImage（选图）→ preprocessing（压缩）→ recognizing（AI 推理）→ lookupNutrition（查库回填）
- 状态圆圈 28×28dp：未到灰色描边，当前紫色+转圈，已完成紫色+勾
- 进度条 `LinearProgressIndicator` value = completedCount/4，minHeight 6dp
- 特殊状态：error 显示"识别失败"，queued 显示"已加入离线队列"，done 4 阶段全 done
- 监听机制：`StateNotifier.addListener(Listener<RecognizeUiState>)` 返回 removeListener 回调，try/finally 配对避免泄漏

### 改动文件
| 文件 | 操作 | 说明 |
|------|------|------|
| `android/app/src/main/res/drawable/ic_launcher_foreground.xml` | 重写 | Google Lens 风四角 L+苹果剪影+扫描线 |
| `lib/features/recognize/recognize_progress_card.dart` | 新建 | 4 阶段进度卡片 widget（~270 行） |
| `lib/features/recognize/recognize_page.dart` | 改 | 替换 loading + addListener 监听状态 + _currentRecognizeState 字段 |
| `test/icon_assets_test.dart` | 改 | group M17→M20，前景断言改为取景框+食物剪影+扫描线 |
| `test/features/recognize_progress_card_test.dart` | 新建 | 10 个 widget 测试 |
| `pubspec.yaml` | 改 | bump 0.19.1+30 → 0.20.0+31 |

### 6 条硬约束复检
1. ✅ `build.gradle.kts` isMinifyEnabled=false + isShrinkResources=false（本改动未触碰）
2. ✅ meal_log.food_item_id 非空外键（本改动未触碰 meal_log 写入路径）
3. ✅ AI 兜底三路径覆盖（本改动仅在 UI 层加监听，控制流不变）
4. ✅ per100g 反算基于 estimatedWeightGMid（本改动未触碰 per100g 逻辑）
5. ✅ SecureConfigStore 无 instance 静态属性（本改动未触碰）
6. ✅ initSentryAndRunApp 命名参数（本改动未触碰）

### 核心不变量
- 识别控制流不变（`RecognizeController.pickAndRecognize` 逻辑不动，仅 UI 层增加状态监听）
- L1 重试/L2 切备/L3 转手动/CircuitBreaker 容灾逻辑不变
- 图标 monochrome 兼容（前景纯白 alpha 通道，系统染色可识别）
- addListener/removeListener 配对（try/finally，避免内存泄漏）

### 用户感知变化
| 场景 | 改前 | 改后 |
|------|------|------|
| App 图标 | M17 同心圆环+圆点（抽象，用户反馈"太难看"） | Google Lens 风四角取景框+苹果+扫描线（精致，谷歌风） |
| 拍照识别等候 | 单一"识别中…"+转圈 | 4 阶段逐步打勾+进度条推进（选图→压缩→AI 推理→查库回填） |
| 识别失败 | 转圈消失+SnackBar | 进度卡片显示"识别失败"特殊文案 |
| 离线入队 | 转圈消失+Toast | 进度卡片显示"已加入离线队列"特殊文案 |

### 待用户执行
1. 装 v0.20.0 APK 验证：
   - 看 App 图标是否像 Google Lens 风格（取景框+苹果+扫描线）
   - 拍照识别时看 4 阶段是否逐步打勾 + 进度条推进
   - 各阶段文案是否准确（选图/压缩/AI 推理/查库回填）
   - 错误态是否显示"识别失败"特殊卡片
   - 离线入队是否显示"已加入离线队列"特殊文案
2. 若图标几何需微调（如苹果太大/扫描线位置），反馈后调整 path 数据

### M19 + M20 一并发布说明
用户明确指令"包括我 M19 的改动也要一并 push 和 tag"：
- M19 commit (da36c5a) 已 push 到 origin/main + tag v0.19.1 已推送
- M20 commit 一并 push + tag v0.20.0
- 远端 main 之前是 v0.8.0 旧线，本地是 v0.19.x 主线，force push 覆盖远端 main 为本地主线
...
### 任务来源
用户指令"全面审查整个项目看看还有哪里有问题，反复严肃检查，别给我出问题，确定没问题 push 和 tag"。用户决策（AskUserQuestion）：P0+P1 全修复 + bump v0.20.1 + tag。
...
- **P0 阻塞项**：HANDOFF.md 第 1/2 节严重不同步（L26 写"0.18.6+25"，L37-40 写"待 push v0.19.0"，实际已是 v0.20.0+31）—— 违反项目规则"会话结束必做：更新 HANDOFF.md 第 2 节"
- **P1 测试缺口**：`glm_4v_provider.dart` 完全无测试 / `qwen_vl_provider.dart` 无直接单测 / `connection.dart` 无直接测试

### 实现方案（TDD 严格循环补测试 + HANDOFF 同步）

**P0 修复**：
- 改 HANDOFF.md L26 第 1 节版本号 0.18.6+25 → 0.20.1+32
- 改 HANDOFF.md L37-40 第 2 节"当前状态"，加入 M19/M20/M21 完成描述 + 当前分支 origin/main

**P1 测试缺口修复**：
- 新建 `test/ai/glm_4v_provider_test.dart`（5 个测试）：构造默认/自定义 modelName + name + promptVersion + VisionProvider 接口实现
- 新建 `test/ai/qwen_vl_provider_test.dart`（11 个测试）：`isRefusalForTest` 静态方法各分支（标准 refusal 字段 + 文本兜底关键词 + 合法 JSON 排除 + 空文本 + 正常响应）

**P1 不补（决策）**：
- `connection.dart` 不补独立测试：依赖 path_provider，测试环境需 mock，drift NativeDatabase 已被 database_test.dart 充分传递覆盖，独立测试收益低成本高
- 27 处 debugPrint 改 logger 留作 M22：需引入统一 logger 封装 + Sentry breadcrumb 集成，工作量大影响 10 个文件

### 改动文件
| 文件 | 操作 | 说明 |
|------|------|------|
| `test/ai/glm_4v_provider_test.dart` | 新建 | 构造 + name + promptVersion 测试（5 个） |
| `test/ai/qwen_vl_provider_test.dart` | 新建 | isRefusalForTest 各分支测试（11 个） |
...
### 核心不变量
- 6 条硬约束全部满足（本改动仅补测试 + 文档，不动生产代码）
- M19/M20 改动区域无回归（测试覆盖 + 文档同步，不改生产逻辑）
- 987 + 16 = 1003 测试全过（analyze No issues）

### 用户感知变化
无（纯测试 + 文档补全，无功能变更，无 UI 变化，v0.20.1 与 v0.20.0 行为一致）

### 待用户执行
1. 装 v0.20.1 APK 验证（与 v0.20.0 行为一致，仅测试+文档补全）
2. 若需进一步改进，参考 P2 候选（留作 M22+）：
   - logger 重构（27 处 debugPrint 改统一 logger + Sentry breadcrumb）
   - 死代码全量扫描（dart_code_metrics 或自定义脚本）
   - table 文件独立测试（8 个表 schema 约束）
   - 3 个 repository 补独立单测（pending_recognition / recognition_feedback / insight）

### 审查遗留 PAT 提醒
HANDOFF.md L45 仍记录 classic PAT `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` 待用户手动删除（GitHub Settings → Tokens）。AI 无法代删，需用户访问 https://github.com/settings/tokens 操作。
